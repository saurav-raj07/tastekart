...
from backend.shared.core import clean_document, generate_order_id, http_error, initialize_database, install_service_auth, menu_items, orders, restaurants_collection, require_role, users
...
def create_order(payload: OrderRequest, request: Request):
    ...
    available = {item["id"]: item for item in menu_items.find(
        {"id": {"$in": item_ids}, "available": True}, {"_id": 0})}
    clean = [(available[item.id], item.quantity)
             for item in payload.items if item.id in available]
    if not clean:
    ...
    orders.insert_one({"id": order_id, "user_id": user["id"], "restaurant_id": restaurant_id, "status": "PLACED", "payment_status": "PENDING", "subtotal": subtotal, "delivery_fee": fee, "total": subtotal +
                      fee, "address": address, "items": [{"id": item["id"], "name": item["name"], "price": item["price"], "quantity": quantity} for item, quantity in clean], "created_at": time.time()})
...
def pay_order(order_id: str, payload: PaymentRequest):
    result = orders.update_one(
        {"id": order_id}, {"$set": {"payment_status": "PAID", "status": "CONFIRMED"}})
...
def update_partner_order(order_id: str, payload: OrderStatusRequest, request: Request):
    ...
    result = orders.update_one({"id": order_id, "restaurant_id": payload.restaurantId}, {
